    ...
    <>
      <nav className={navOpen ? 'nav-open' : ''}>
        ...
        <div className="nav-links">
          ...
          {role === 'admin' ? (
            <Link href="/admin" style={{ color: 'var(--accent)', fontWeight: 600 }} onClick={() => setNavOpen(false)}>Admin Dashboard</Link>
          ) : role === 'client' ? (
            <Link href="/client" style={{ color: 'var(--accent)', fontWeight: 600 }} onClick={() => setNavOpen(false)}>My Project</Link>
          ) : (
            <Link href="/client/login" style={{ color: 'var(--accent)', fontWeight: 600 }} onClick={() => setNavOpen(false)}>Client Login</Link>
          )}
        </div>
      </nav>
